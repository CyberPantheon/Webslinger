...
async function confirmAndExploit({ targetUrl, paramName, httpMethod, originalParams, baseResponse, initialTechnique, initialPayload, context }) {
  ...
  if (globalOptions.booleanBased !== false && !confirmed) {
    ...
    for(let i=0; i < (payloads.booleanBased || []).length; i+=2) {
        if (payloads.booleanBased[i+1]) booleanPayloadPairs.push([payloads.booleanBased[i], payloads.booleanBased[i+1]]);
    }
    for (const [true_template, false_template] of booleanPayloadPairs) {
        if (stopRequested) break;
            ...
            try {
              ...
              if (potentialTechnique) {
                await confirmAndExploit({
                  ...
                  initialTechnique: potentialTechnique, initialPayload: probe,
